  ...
  server <- function(input, output, session) {
    ...
    heritageCalculator<-function(df_heritagea,df_heritageb,df_heritagec)
    {
      ...
      if(input$checkbox2&&input$checkbox1==1){
        ...
        value1<- heritage_total[3,1]*heritage_total[3,2]*df_heritagec[1,1]
        value2<- heritage_total[2,1]*heritage_total[4,2]*df_heritagec[1,1]
        print("Night Stays Part 2")
    ...
    output$downloadData <- downloadHandler( 
      ...
      #The name just shows the date. I should change the name in the near future. 
        ...
        #   #Part 3 Tourism CHECK ALL OF THIS ALLOCATION
          ...
          #  print("Heritage values:")
          #  print(heritageValues)
            
          #  total_output[365,3]<<-heritageValues[1] ## df_heritage<<- reactiveVal(as.data.frame(matrix(0, nrow = 5, ncol = 2)))
           #heritageCalculator will combine the heritage tables together, analyze the checkbox and assign values. 
          #  total_output[364,3]<<-heritageValues[2]
            
          #  total_output[394,3]<<-heritageValues[3]#I attempted to pick the Household Sector
       ...
       #       print("we;re past the first SPENDING one.")
   ...
   output$regions<- renderUI({
          ...
          tabItem(tabName ="first", class = "active",
                ...
                ")),
                 ...
                 actionButton(inputId = "rehab_button", label = NULL, style = "width: 450px; height: 250px;
                      background: url('rehab2.jpg');  background-size: cover; background-position: center;"),
    ...
    observeEvent(input$buttondefault, {
      ...
      df_heritagec<-df_heritagec()
      df_heritagec[1,1]<-0
      heritageCalculator(df_heritagea,df_heritageb,df_heritagec)
      })
