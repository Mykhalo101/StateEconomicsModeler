...
server <- function(input, output, session) {
  ...
  output$downloadData <- downloadHandler( 
    ...
    content = function(file) {
      ...
      test<<-list_of_datasets
      print("B")
    #  sheet2 <- createSheet(final_file, sheetName = "Super Sectors")
      print("C")
     #  sheet3 <- createSheet(final_file, sheetName = "3-digit Industry")
       print("D")
     # sheet4 <- createSheet(final_file, sheetName = "Direct Effects")
      print("E")
      writeData(wb,"Detailed Industry",(list_of_datasets[[1]]))
      print("F")
      saveWorkbook(wb, file="bloop.xlsx",overwrite=TRUE)
      #write_xlsx(wb, file)
